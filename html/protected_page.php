...
$cmd = pg_query($conn, "SELECT model, tip, godinapr, motor, count(*) FROM oglasi GROUP BY model, tip, godinapr, motor HAVING count(*) > 30 ORDER BY COUNT(*) DESC;");
if (!$cmd) {
    echo "error2";
}
...
while ($row = pg_fetch_row($cmd)) {
    ...
    $modeli[$modeliLength][3] = $row[3];
    $modeliLength++;
}

for ($i = 0; $i < $modeliLength; $i++) {
    $sql = "SELECT avg(cijena), stddev(cijena) FROM oglasi WHERE model = '" . $modeli[$i][0] . "' AND tip = '" . $modeli[$i][1] . "' AND godinapr = " . $modeli[$i][2] . " AND motor = '" . $modeli[$i][3] . "' ; ";
    ...
    while ($row = pg_fetch_row($cmd)) {
        if ($row[1] != 0) {
            ...
            //echo $targetPrice."<br/>";
            $sql = "SELECT * FROM oglasi WHERE cijena < $targetPrice AND model = '" . $modeli[$i][0] . "' AND tip = '" . $modeli[$i][1] . "' AND godinapr = " . $modeli[$i][2] . " AND motor = '" . $modeli[$i][3] . "' ; ";
